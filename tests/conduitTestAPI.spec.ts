...
test('Conduit MOCK test API', async ({ page }) => {
  ...
  await expect(page.locator('.tag-list')).toContainText('Playstation')
  await expect(page.locator('app-article-list h1').first()).toContainText('This is a MOCK test title')
  await expect(page.locator('app-article-list p').first()).toContainText('This is a MOCK description')
});

test('Create article using API & delete using UI', async ({page, request}) => {
...
test('Create article using UI & delete using API', async ({page, request}) => {
